...
let skymapUpdateInterval = null; // 星座図のリアルタイム更新用interval
let issNotificationInterval = null; // ISS通知チェック用interval
let notifiedPasses = new Set(); // 通知済みのパスを記録（重複通知を防ぐ）
let issNotificationPermission = 'default'; // 通知権限の状態
...
(function() {
    if ('Notification' in window && Notification.permission === 'default') {
        console.log('ISS通過通知を有効にするには、通知を許可してください');
    }
    if ('Notification' in window) {
        issNotificationPermission = Notification.permission;
    }
    window.addEventListener('beforeunload', () => {
        stopISSNotificationCheck();
    });
})();
